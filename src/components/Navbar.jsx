...
const Navbar = () => {
  ...
  const navLinks = [
    ...
    { name: "Tests", to: "/#tests", icon: <BookOpen className="h-4 w-4" /> },
    { name: "Stats", to: "/#stats", icon: <BarChart2 className="h-4 w-4" /> },
    { name: "Testimonials", to: "/#testimonials", icon: <Star className="h-4 w-4" /> },
    { name: "Contact", to: "/#contact", icon: <Contact className="h-4 w-4" /> }
  ];
  ...
  const checkAuthStatus = async () => {
    try {
      ...
      if (token) {
        ...
        setUser(response.data); // ✅ keep user object, not token
      }
    } catch (error) {
  ...
  useEffect(() => {
    const handleLogin = (event) => {
      if (event.detail && event.detail.user && event.detail.token) {
        setUser(event.detail.user); // ✅ user object
        localStorage.setItem('token', event.detail.token); // ✅ token in storage
      }
    };
